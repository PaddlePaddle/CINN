...
TEST(test03_conv, schedule) {
 ...
 auto kernel = Compute(
    {oc_chunk, ic_chunk, h_f, w_f, ic_bn, oc_bn},
        [=](Expr occ, Expr icc, Expr h, Expr w, Expr icb, Expr ocb) { return weights(occ * oc_bn + ocb, icc * ic_bn + icb, h, w); },
        UniqName("kernel_vec"));
  ...
  auto input_pad = Compute(
      ...
      [=](Expr bs, Expr cc, Expr yy, Expr xx, Expr vc) {
        auto cond =
            lang::logic_and({yy >= pad_h, yy - pad_h < h_in, xx >= pad_w, xx - pad_w < w_in});
  ...
  stages[CC_init]->ComputeAt2(stages[CC], 1);
  ...
  auto* A        = cinn_buffer_t::new_(cinn_device_kind_t::cinn_x86_device, cinn_float32_t(), {n.as_int32(), c_in.as_int32(), h_in.as_int32(), w_in.as_int32()}, 32);
  auto* B        = cinn_buffer_t::new_(cinn_device_kind_t::cinn_x86_device, cinn_float32_t(), {c_out.as_int32(), c_in.as_int32(), h_f.as_int32(), w_f.as_int32()}, 32);
  auto* C        = cinn_buffer_t::new_(cinn_device_kind_t::cinn_x86_device, cinn_float32_t(), {n.as_int32(), c_out.as_int32(), out_h, out_w}, 32);
  auto* input_pad_buffer = cinn_buffer_t::new_(cinn_device_kind_t::cinn_x86_device, cinn_float32_t(), {n.as_int32(), c_in.as_int32(), input_pad_h, input_pad_w}, 32);
  auto* packed_out_buffer = cinn_buffer_t::new_(cinn_device_kind_t::cinn_x86_device, cinn_float32_t(), {n.as_int32(), c_out.as_int32()/32, out_h, out_w, 32}, 32);
