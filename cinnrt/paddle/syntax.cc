...
namespace cinnrt {
namespace paddle {
using cinnrt::paddle::Scope;

void Instruction::PrepareOutputs() {
  auto* op_def = cinnrt::paddle::OpRegistry::Global()->Find(get()->op_type);
...
Instruction::Instruction(std::string_view op_type, const std::vector<Variable>& inputs, Program* parent)
    : cinnrt::common::Shared<_Instruction_>(cinnrt::common::make_shared<_Instruction_>()) {
  get()->op_type        = op_type;
...
std::string _Instruction_::debug_string() const {
  struct Visit {
    ...
    void operator()(const std::vector<int>& x) { s_ << "[" + cinnrt::cinn::Join(x, ",") + "]"; }
    void operator()(const std::vector<float>& x) { s_ << "[" + cinnrt::cinn::Join(x, ",") + "]"; }
    void operator()(const std::vector<bool>& x) { s_ << "[" + cinnrt::cinn::Join(x, ",") + "]"; }
    void operator()(const std::vector<std::string>& x) { s_ << "[" + cinnrt::cinn::Join(x, ",") + "]"; }
  };
  ...
  ss << cinnrt::cinn::Join(output_names, ", ");
  ss << " = ";
  ss << op_type;
  ss << "(";
  ss << cinnrt::cinn::Join(input_names, ", ");
  if (!attrs.empty()) ss << ", ";
