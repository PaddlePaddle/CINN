...
using cinnrt::cinn::Join;
using cinnrt::cinn::TransValidVarName;
using cinnrt::common::Float;
...
void PaddleModelToProgram::AddOpMapper_scale() {
  op_mappers_["scale"] = [&](const ::cinnrt::paddle::cpp::OpDesc& op_desc) {
    ...
    if (op_desc.HasAttr("scale")) {  // the old model format
      ...
      auto& scale_tensor = std::get<cinnrt::paddle::Tensor>(*scale_tensor_var);
      scale              = scale_tensor->mutable_data<float>(cinnrt::common::DefaultHostTarget())[0];
    }
    ...
    std::unordered_map<std::string, cinnrt::paddle::NodeAttr::attr_t> attrs;
    attrs["scale"] = scale;
    attrs["bias"]  = bias;
    auto out       = program_->scale(x, attrs);
    CHECK_EQ(op_desc.Output("Out").size(), 1UL);
    auto out_name = op_desc.Output("Out").front();
    AddVar(cinnrt::cinn::TransValidVarName(out_name), out);
...
void PaddleModelToProgram::AddOpMapper_mul() {
  op_mappers_["mul"] = [&](const ::cinnrt::paddle::cpp::OpDesc& op_desc) {
    ...
    auto y             = GetVar(cinnrt::cinn::TransValidVarName(y_name));
    ...
    VLOG(4) << "x shape: " << cinnrt::cinn::Join(x->shape, ",");
    VLOG(4) << "y shape: " << cinnrt::cinn::Join(y->shape, ",");
