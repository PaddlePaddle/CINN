...
#define TRANS_VAR_ANY_WITH_CPP_IMPL(T)                                                                    \
  template <>                                                                                             \
  void TransformVarDescCppToAny<T>(const ::cinnrt::paddle::cpp::VarDesc &cpp_desc, T *any_desc) { \
...
template <>
void TransformVarDescAnyToCpp<::cinnrt::paddle::pb::VarDesc>(
    const ::cinnrt::paddle::pb::VarDesc &any_desc, ::cinnrt::paddle::cpp::VarDesc *cpp_desc) {
...
void OpInputsAnyToCpp(const OpDescType &any_desc, ::cinnrt::paddle::cpp::OpDesc *cpp_desc) {
...
template <typename OpDescType>
void OpInputsCppToAny(const ::cinnrt::paddle::cpp::OpDesc &cpp_desc, OpDescType *any_desc) {
...
template <typename OpDescType>
void OpOutputsAnyToCpp(const OpDescType &any_desc, ::cinnrt::paddle::cpp::OpDesc *cpp_desc) {
...
template <typename OpDescType>
void OpOutputsCppToAny(const ::cinnrt::paddle::cpp::OpDesc &cpp_desc, OpDescType *any_desc) {
...
template <typename OpDescType>
void OpAttrsAnyToCpp(const OpDescType &any_desc, ::cinnrt::paddle::cpp::OpDesc *cpp_desc) {
  using AttrType = ::cinnrt::paddle::cpp::OpDescAPI::AttrType;
...
template <typename OpDescType>
void OpAttrsCppToAny(const ::cinnrt::paddle::cpp::OpDesc &cpp_desc, OpDescType *any_desc) {
  using AttrType = ::cinnrt::paddle::cpp::OpDescAPI::AttrType;
  auto set_attr  = [&](const std::string &name, AttrType type) {
    switch (type) {
...
#define TRANS_OP_ANY_WITH_CPP_IMPL(T)                                                                   \
  template <>                                                                                           \
  void TransformOpDescAnyToCpp<T>(const T &any_desc, ::cinnrt::paddle::cpp::OpDesc *cpp_desc) { \
    ...
    OpAttrsAnyToCpp<T>(any_desc, cpp_desc);                                                             \
                                                                                                        ...
                                                                                                        \
  template <>                                                                                           \
  void TransformOpDescCppToAny<T>(const ::cinnrt::paddle::cpp::OpDesc &cpp_desc, T *any_desc) { \
...
#define TRANS_BLOCK_ANY_WITH_CPP_IMPL(T, NT)                                                                          \
  template <>                                                                                                         \
  void TransformBlockDescAnyToCpp<NT::T>(const NT::T &any_desc, ::cinnrt::paddle::cpp::BlockDesc *cpp_desc) { \
    NT::T desc = any_desc;                                                                                            \
    ...
    for (size_t i = 0; i < desc.OpsSize(); ++i) {                                                                     \
      ...
      auto *cpp_op_desc = cpp_desc->AddOp<::cinnrt::paddle::cpp::OpDesc>();                                   \
    ...
    for (size_t i = 0; i < desc.VarsSize(); ++i) {                                                                    \
      ...
      auto *cpp_var_desc = cpp_desc->AddVar<::cinnrt::paddle::cpp::VarDesc>();                                \
      TransformVarDescAnyToCpp(any_var_desc, cpp_var_desc);                                                           \
    }                                                                                                                 \
  }                                                                                                                   \
                                                                                                                      \
  template <>                                                                                                         \
  void TransformBlockDescCppToAny<NT::T>(const ::cinnrt::paddle::cpp::T &cpp_desc, NT::T *any_desc) {         \
    auto desc = cpp_desc;                                                                                             \
    ...
    any_desc->ClearOps();                                                                                             \
    for (size_t i = 0; i < desc.OpsSize(); ++i) {                                                                     \
      auto *cpp_op_desc = desc.GetOp<::cinnrt::paddle::cpp::OpDesc>(i);                                       \
    ...
    any_desc->ClearVars();                                                                                            \
    for (size_t i = 0; i < desc.VarsSize(); ++i) {                                                                    \
      auto *cpp_var_desc = desc.GetVar<::cinnrt::paddle::cpp::VarDesc>(i);                                    \
...
#define TRANS_PROGRAM_ANY_WITH_CPP_IMPL(T, NT)                                                                  \
  ...
  void TransformProgramDescAnyToCpp<NT::T>(const NT::T &any_desc,                                               \
    ...
    for (size_t i = 0; i < desc.BlocksSize(); ++i) {                                                            \
      ...
      auto *cpp_block_desc = cpp_desc->AddBlock<::cinnrt::paddle::cpp::BlockDesc>();                    \
      TransformBlockDescAnyToCpp(any_block_desc, cpp_block_desc);                                               \
    }                                                                                                           \
  }                                                                                                             \
                                                                                                                \
  template <>                                                                                                   \
  void TransformProgramDescCppToAny<NT::T>(const ::cinnrt::paddle::cpp::T &cpp_desc, NT::T *any_desc) { \
    auto desc = cpp_desc;                                                                                       \
    if (desc.HasVersion()) {                                                                                    \
    ...
    for (size_t i = 0; i < desc.BlocksSize(); ++i) {                                                            \
      auto *cpp_block_desc = desc.GetBlock<::cinnrt::paddle::cpp::BlockDesc>(i);                        \
...
TRANS_VAR_ANY_WITH_CPP_IMPL(::cinnrt::paddle::pb::VarDesc);
TRANS_OP_ANY_WITH_CPP_IMPL(::cinnrt::paddle::pb::OpDesc);
TRANS_BLOCK_ANY_WITH_CPP_IMPL(BlockDesc, ::cinnrt::paddle::pb)
TRANS_PROGRAM_ANY_WITH_CPP_IMPL(ProgramDesc, ::cinnrt::paddle::pb)
