...
struct _Variable_ : public cinnrt::common::Object {
  std::string id;
  cinnrt::common::Type type;
 ...
 */
struct Variable : public cinnrt::common::Shared<_Variable_> {
  /**
   * Constructor.
   * @param id_hint The identifier of the variable, if null, a random ID will be assigned.
   */
  explicit Variable(std::string_view id_hint = "")
      : cinnrt::common::Shared<_Variable_>(cinnrt::common::make_shared<_Variable_>()) {
    if (!id_hint.empty()) cinnrt::CheckVarNameValid(id_hint);
    get()->id = id_hint.empty() ? cinnrt::common::Context::Global().NewName("var") : id_hint;
  }
...
class Placeholder {
 public:
   ...
   */
  Placeholder(const cinnrt::common::Type& type, const std::vector<int>& shape, std::string_view id_hint = "") {
    if (!id_hint.empty()) cinnrt::CheckVarNameValid(std::string(id_hint));
    id_         = id_hint.empty() ? cinnrt::common::Context::Global().NewName("placeholder") : id_hint;
    var_        = Variable(id_);
    var_->shape = shape;
    var_->type  = type;
  }
  ...
  cinnrt::common::Type type() const { return var_->type; }
 ...
 */
struct _Instruction_ : public cinnrt::common::Object {
  using attr_t = cinnrt::paddle::AttrType;
 ...
 */
struct Instruction : public cinnrt::common::Shared<_Instruction_> {
 ...
 */
struct Program {
  using attr_t = cinnrt::paddle::NodeAttr::attr_t;
