...
class Variant {
  ...
  template <typename T>
  static constexpr bool IsVariant = std::is_same<std::decay_t<T>, Variant>::value;
...
decltype(auto) visitHelper(F&& f, Variant&& v, std::integral_constant<int, std::decay_t<Variant>::kNTypes>) {
...
decltype(auto) visitHelper(F&& f, Variant&& v, std::integral_constant<int, N>) {
  ...
  if (auto* t = v.template get_if<T>()) {
    ...
    return visitHelper(std::forward<F>(f), std::forward<Variant>(v), std::integral_constant<int, N + 1>());
  }
}
