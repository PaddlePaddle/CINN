...
Variable NetBuilder::Pool2d(const Variable& a,
  ...
  std::vector<int> new_ksize{ksize};
  if (new_ksize.size() == 1) {
    new_ksize.insert(new_ksize.end(), ksize.begin(), ksize.end());
  }
  CHECK_EQ(new_ksize.size(), 2) << "Kernel_size length must be 1 or 2, but got: " << ksize.size();
  CHECK(new_ksize[0] > 0 && new_ksize[1] > 0) << "the value of kernel size for pool2d should greater than 0.";
  // Transform stride
  ...
  auto new_paddings = UpdatePool2dPaddings(
      paddings, a->shape, new_ksize, new_strides, global_pooling, adaptive, padding_algorithm, new_data_format);
  // Update kernel_size if global_pooling is true
  new_ksize = UpdatePool2dKernelSize(a->shape, new_ksize, global_pooling, new_data_format);
...
Variable NetBuilder::Pool2dGrad(const Variable& x,
  ...
  // Transform paddings
  auto new_paddings = UpdatePool2dPaddings(
      paddings, x->shape, new_ksize, new_strides, global_pooling, adaptive, padding_algorithm, new_data_format);
  // Update kernel_size if global_pooling is true
  new_ksize = UpdatePool2dKernelSize(x->shape, new_ksize, global_pooling, new_data_format);
