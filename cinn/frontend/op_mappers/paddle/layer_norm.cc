...
void LayerNormOpMapper(const paddle::cpp::OpDesc& op_desc, const OpMapperContext& ctx) {
  ...
  auto ele_num   = builder->FillConstant({left}, static_cast<float>(right), common::UniqName("layer_norm_ele_num"));
  ...
  auto zero      = builder->FillConstant({left}, 0.f, common::UniqName("layer_norm_zero"));
  ...
  auto epsilon_var      = builder->FillConstant({left}, epsilon, common::UniqName("layer_norm_epsilon"));
