...
TEST(sign, sign_ok) {
  ...
  std::vector<float> x_data{-2, -0, NaN, +0, 2};
  std::vector<float> output_ref{-1.0, 0, NaN, 0, 1};
  ...
  auto* sign          = OpMapperRegistry::Global()->Find(op_type);
  CHECK_NOTNULL(sign);
  ...
  std::vector<int> x_shape = {1, 5};
  create_input(x_name, Float(32), x_shape);

  // output
  std::string out_name = "sign_0.tmp_0";
