...
TEST(Decomposer, elementwise_add_grad_bcast1) {
  std::vector<int> x_shape = {32, 64, 32, 32};
  std::vector<int> y_shape = {64};
...
TEST(Decomposer, elementwise_add_grad_bcast2) {
  std::vector<int> x_shape = {32, 16};
  std::vector<int> y_shape = {1};

  NetBuilder builder("elementwise_add_grad");
  auto dout      = builder.CreateInput(Float(32), {32, 16});
  auto out_grads = builder.elementwise_add_grad(dout, x_shape, y_shape);
