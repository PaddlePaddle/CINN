...
TEST(net_build, program_execute_fc) {
  ...
  int hidden_size = 768;
  // x
  auto A = builder.CreateInput(Float(32), {batch_size, seq_len, hidden_size}, "A");
  // x * x
  auto B = builder.Multiply(A, A);
  // sum x
  auto C = builder.ReduceSum(A, {2});
  // sum x*x
  auto D = builder.ReduceSum(B, {2});
  // constant w
  auto E = builder.FillConstant<float>({batch_size, seq_len}, hidden_size, "E");
  // mean
  auto F  = builder.Divide(C, E);
  auto FF = builder.BroadcastTo(F, {batch_size, seq_len, hidden_size}, {0, 1});
  // output mean
  auto reshape_mean = builder.Reshape(F, {65536});
  auto out_mean = builder.Identity(reshape_mean);
  // mean x*x
  auto G = builder.Divide(D, E);
  // mean * mean
  auto H = builder.Multiply(F, F);
  // var^2
  auto I = builder.Subtract(G, H);
  // output variance
  auto reshape_var = builder.Reshape(I, {65536});
  auto out_var = builder.Identity(reshape_var);
  // eps
  auto J = builder.FillConstant<float>({batch_size, seq_len}, 1e-10f, "J");
  // eps + delta
  auto K = builder.Add(I, J);
  // var
  auto L  = builder.Sqrt(K);
  auto LL = builder.BroadcastTo(L, {batch_size, seq_len, hidden_size}, {0, 1});
  // x - mean
  auto M = builder.Subtract(A, FF);
  // /var
  auto N = builder.Divide(M, LL);
  // weight
  auto weight = builder.FillConstant<float>({hidden_size}, 1.0, "weight");
  auto ww = builder.BroadcastTo(weight, {batch_size, seq_len, hidden_size}, {2});
  auto O = builder.Multiply(N, ww);
  auto bias = builder.FillConstant<float>({hidden_size}, 0.0, "bias");
  auto bb = builder.BroadcastTo(bias, {batch_size, seq_len, hidden_size}, {2});
  auto P = builder.Add(O, bb);
