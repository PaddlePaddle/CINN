...
class IsReachablePredicator final {
 public:
  ...
  IsReachablePredicator(const NodeDepthGetterType& MinDepth4Node,
      ...
      : MinDepth4Node_(MinDepth4Node), MaxDepth4Node_(MaxDepth4Node), VisitNextNodes_(VisitNextNodes) {}

  bool operator()(NodeType src, NodeType dst, const NodeHandlerType& HandleVisited) const {
    const size_t dst_max_depth = MaxDepth4Node_(dst);
    bool detect_reachable      = false;
    BfsVisitor<NodeType> bfs_visitor([&](NodeType node, const NodeHandlerType& Handler) {
