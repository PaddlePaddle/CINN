...
class SccVisitor final {
 public:
  ...
  SccVisitor(const NodesVisitorType& VisitPrevNodes, const NodesVisitorType& VisitNextNodes)
  ...
  void operator()(NodeIt begin, NodeIt end, const SccHandlerType& SccHandler) const {
    const std::list<NodeType>& dfs_ordered_nodes = [&]() {
      ...
      visitor(
          ...
          /*on push*/ [](NodeType) {},
          /*on pop*/ [&](NodeType node) { dfs_ordered_nodes.push_front(node); });
      return dfs_ordered_nodes;
    }();
    std::unordered_map<NodeType, NodeType> node2root;
    const auto& VisitPrevNode = [&](NodeType node, const NodeHandlerType& NodeHandler) {
