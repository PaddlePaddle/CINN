...
class DfsVisitor final {
 public:
  ...
  DfsVisitor(const NodesVisitorType& VisitNextNodes) : VisitNextNodes_(VisitNextNodes) {}
  ...
  void operator()(NodeIt begin,
    ...
    const auto& TryPush = [&](NodeType node) {
      if (discovered.count(node) == 0) {
        ...
        VisitNextNodes_(node, [&](NodeType next_node) { stack.top().consumers.push(next_node); });
      }
    };
