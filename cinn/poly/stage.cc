...
void Stage::ComputeAt(Stage *other, int level) {
  ...
  for (int i = 0; i <= level; i++) {
    if (IsUnrolled(i))
      AddForloopInfo(i, StageForloopInfo{ir::ForType::Unrolled, DeviceAPI::UNK, 0});
    else
...
Iterator Stage::Fuse(const Iterator &level0, const Iterator &level1) {
  ...
  conds.emplace_back(utils::StringFormat("0<=%s<=%d", level1.id.c_str(), level1_max_val - 1));
...
void Stage::Unroll(int level) {
  ...
  int removed_axes_counts = isl_get_precending_removed_axes_counts(transformed_domain.get(), level);
  VLOG(3) << "removed_axes_counts are " << removed_axes_counts << " before axis " << ith_dim_name(level);
  // LOG(INFO) << "Stage of " << id() << " insert unroll_info with level = " << level << " and level -
  // removed_axes_counts is " << level - removed_axes_counts;
  unroll_info_.insert(level - removed_axes_counts);
  if (HaveForloopInfo(level) && forloop_infos_[level - removed_axes_counts].for_type == ir::ForType::Default)
    AddForloopInfo(level, StageForloopInfo{ir::ForType::Unrolled, DeviceAPI::UNK, 0});
}

bool Stage::IsUnrolled(int level) {
  CHECK_GE(level, 0);
  auto transformed_domain = this->transformed_domain();
  if (isl_is_removed_axis(transformed_domain.get(), level)) {
    VLOG(1) << "Unrolling for-1 has no sense, skip it";
  ...
  // LOG(INFO) << "Stage of " << id() << " insert unroll_info with level = " << level << " and level -
  // removed_axes_counts is " << level - removed_axes_counts;
  if (unroll_info_.count(level - removed_axes_counts) > 0) return true;
  return false;
}
...
bool Stage::HaveForloopInfo(int level) {
  int num_levels = isl_map_dim(transform_.get(), isl_dim_out);
  CHECK_GE(level, 0);
  CHECK_LT(level, num_levels);
  auto transformed_domain = this->transformed_domain();
  ...
  if (forloop_infos_.count(level - removed_axes_counts) > 0) return true;
  return false;
}
