...
void Stage::ChangeDomain(Stage *other, int level) {
  ...
  LOG(INFO) << "CollectTensorIndex is : " << this->tensor()->name;
  for (auto &i : indices) {
    LOG(INFO) << i;
  }
...
void Stage::ComputeAt5(Stage *other, int level) {
  ...
  LOG(INFO) << "read_access is 3 : " << read_access;
  auto indices = optim::CollectTensorIndex(&(other->expr_), this->tensor()->name);
  RemoveDuplicate(indices);
  if (indices.empty()) {
    return;
  ...
  for (auto &i : indices[0]) {
    ...
    if (target_dims_set.count(str_name) > 0) {
      ...
      for (auto &i : target_dims_set) {
        LOG(INFO) << "i in target_dims_set is : " << i;
      }
      LOG(INFO) << "str_name is : " << str_name;
    }
  ...
  LOG(INFO) << "read_access is 4 : " << read_access;
  read_access =
      isl::manage(isl_map_set_tuple_name(read_access.release(), isl_dim_out, isl_set_get_tuple_name(domain().get())));
  LOG(INFO) << "read_access is 4.5 : " << read_access;
  std::string read_access_str = isl_map_to_str(read_access.get());
  LOG(INFO) << "read_access_str is 4.5 : " << read_access_str;
  isl::map read_access2(read_access.ctx(), read_access_str);
  LOG(INFO) << "read_access2 is 4.5 : " << read_access2;
  read_access2 = isl::manage(isl_map_reverse(read_access2.release()));
  LOG(INFO) << "read_access2 is 5 : " << read_access2;
  LOG(INFO) << "write_access is 5 : " << write_access;

  auto new_map = isl::manage(isl_map_apply_range(write_access.release(), read_access2.release()));
  LOG(INFO) << "new_map is : " << new_map;
  ...
  LOG(INFO) << "new_target_transform is : " << new_target_transform;
  isl::map f_target_transform(other->transform().ctx(), isl_map_to_str(new_target_transform.get()));
  LOG(INFO) << "f_target_transform is : " << f_target_transform;
  ...
  LOG(INFO) << "trans_res is : " << trans_res;

  /*   std::string trans_res_str = isl_map_to_str(trans_res.get());
    std::string target_trans_str;
      target_trans_str = isl_map_to_str(other->transform().get());
      LOG(INFO) << "target_trans_str 1 is : " << target_trans_str;
      size_t pos1 = target_trans_str.find_last_of("]");
      target_trans_str.insert(pos1, ", 1");
      size_t pos2 = trans_res_str.find_last_of("]");
      trans_res_str.insert(pos2, ", 0");
    LOG(INFO) << "target_trans_str is : " << target_trans_str;
    LOG(INFO) << "trans_res_str is : " << trans_res_str; */

  /*   std::string target_trans_str = "{ C[i, j] -> C[i' = i, j' = j] }";
    std::string trans_res_str = "{ B_read_cache[i, j] -> B_read_cache[i' = i, j' = j] : 0 <= i <= 99 and 0 <= j <= 199
    }"; isl::map trans_target(other->domain().ctx(), target_trans_str); isl::map trans_res2(domain().ctx(),
    trans_res_str); */

  /*   std::string tuple_name = "";
    trans_target = isl::manage(isl_map_set_tuple_name(trans_target.release(), isl_dim_out, tuple_name.c_str()));
    trans_res2 = isl::manage(isl_map_set_tuple_name(trans_res2.release(), isl_dim_out, tuple_name.c_str())); */
  transform_ = trans_res;
...
void Stage::ComputeAt2(Stage *other, int level) {
  // TODO(Superjomn) Check there are data dependency between `self` and `other`, or the `ComputeAt` is meaningless.
  LOG(INFO) << "In computeAt2, level is : " << level;
  this->ShowISL();
  other->ShowISL();
  LOG(INFO) << other->tensor()->tensor_store_expanded_body();
  LOG(INFO) << "ComputeAt2 init";
