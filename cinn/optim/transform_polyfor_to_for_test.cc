  ...
  for (int32_t i_outer = 0; i_outer < 64; i_outer += 1) {
    for (int32_t i_inner = 0; i_inner < 8; i_inner += 1) {
      for (int32_t j_outer = 0; j_outer < 63; j_outer += 1) {
        for (int32_t j_inner = 0; j_inner < cinn_min(8, ((j_outer * -8) + 500)); j_inner += 1) {
          C__reduce_init[((i_inner * 500) + ((i_outer * 4000) + (j_inner + (j_outer * 8))))] = 0;
          for (int32_t k0 = 0; k0 < 200; k0 += 1) {
            C[((i_inner * 500) + ((i_outer * 4000) + (j_inner + (j_outer * 8))))] = fma(A[((i_inner * 200) + ((i_outer * 1600) + k0))], B[(j_inner + ((j_outer * 8) + (k0 * 500)))], C[((i_inner * 500) + ((i_outer * 4000) + (j_inner + (j_outer * 8))))]);
          };
        };
      };
    };
