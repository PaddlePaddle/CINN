...
void TransformGpuForloops(const forloop_infos_t &forloop_infos,
  ...
  for (auto &i : traverse_order) {
    if (forloop_infos.count(i) == 0) continue;
    LOG(INFO) << "Begin MarkGpuForloop: " << i;
...
void OptimizeExprGPU(Expr *expr) {
  ...
  for (auto &for_loop : find_bind_forloops) {
    ...
    LOG(INFO) << "Begin find for loop:" << for_loop;
  ...
  std::unordered_set<std::string> resized_buffer;
  for (auto &i : tensor_traverse_order) LOG(INFO) << "In tensor_traverse_order, it is : " << i;
  for (auto &i : global_tensor_map) LOG(INFO) << "In global_tensor_map, it is : " << i.first;
  for (auto &i : forloop_infos) {
    LOG(INFO) << "In forloop_infos, it is : " << i.first;
    for (auto &j : i.second) {
      LOG(INFO) << "In forloop_infos.second, it is : " << j.first;
    }
  }
