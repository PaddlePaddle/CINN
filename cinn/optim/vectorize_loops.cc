...
class TensorVectorizeExcluder : public ir::IRMutator<const Expr *> {
 public:
  ...
  bool CanBeVectorized(const std::string &tensor_name) const { return !excluded_tensors_.count(tensor_name); }
...
// find tensors accessed sequentially in vectorized for-loop
// and substitue the corresponding cuda built-in vector type for them
class CudaVectorizer : public IRMutator<Expr *> {
 ...
 public:
  ...
  void Visit(const Load *op, Expr *expr) override {
    ...
    auto *tensor = node->tensor.As<ir::_Tensor_>();
    CHECK(tensor);
    if (tensor_excluder_.CanBeVectorized(tensor->name)) {
      VLOG(5) << "Vectorizing tensor:" << tensor->name;
      if (!tensor2vectorized_vars_.count(tensor->name)) AppendCast(node->tensor, node->indices);
...
#define GET_CUDA_VECTOR_TYPE_NAME(pred_expr, scalar_name) \
  if (pred_expr) {                                        \
    return scalar_name + std::to_string(factor_);         \
  }
...
#undef GET_CUDA_VECTOR_TYPE_NAME
  ...
  Type TypeVectorizing(Type raw_type, bool is_const) {
    Type vector_type(Type::type_t::Customized, raw_type.bits(), factor_);
    std::string type_name = common::customized_type::kcuda_builtin_vector_t;
    type_name += GetTypeName(raw_type);
    vector_type.set_customized_type(type_name);
  ...
  void AppendCast(Expr tensor, const std::vector<Expr> &indices) {
    ...
    Type vector_type            = TypeVectorizing(tensor->type().ElementOf(), is_const);
