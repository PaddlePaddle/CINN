...
std::map<std::string, ir::Tensor> InitialAssignBuffer(Expr* expr,
  ...
  auto tensor_exprs = ir::CollectIRNodes(*expr, [](const Expr* x) {
    auto* t = x->as_tensor();
    return t && (!t->meta.compute_inline) && !t->buffer.defined();
  });
  ...
  for (auto& item : tensor_map) {
    ...
    if (!item.second->meta.tensors_to_share_buffer_with.empty()) {
      for (auto& other : item.second->meta.tensors_to_share_buffer_with) {
