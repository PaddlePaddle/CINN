...
TEST(Tensor, reduce) {
  ...
  {
    auto C = Compute(
        A->shape, [=](const std::vector<Expr>& axis) { return lang::Sum(A(reduce_axis) + 1.f); }, "C", {reduce_axis});
  ...
  {
    auto C = Compute(
        A->shape, [=](const std::vector<Expr>& axis) { return lang::Mul(A(reduce_axis) + 1.f); }, "C", {reduce_axis});
    ASSERT_TRUE(C->has_expression());
