...
__device__ inline bool cinn_warp_shuffle_all_internal(const bool value) {
  cinn_warp_shuffle_internal_kernel(value, cinn_all);
}
__device__ inline bool cinn_warp_shuffle_any_internal(const bool value) {
  cinn_warp_shuffle_internal_kernel(value, cinn_any);
}
...
#define cinn_block_reduce_internal_kernel(value, init_value, cinn_warp_shuffle_internal) \
  ...
  __shared__ decltype(value) tmp[32];                                                    \
  if (warp_id == 0) {                                                                    \
    tmp[threadIdx.x] = init_value;                                                       \
  }                                                                                      \
  auto tmp_val = cinn_warp_shuffle_internal(value);                                     \
  if (blockDim.x <= 32) {                                                                \
    return tmp_val;                                                                      \
  }                                                                                      \
  __syncthreads();                                                                       \
...
__device__ inline bool cinn_block_reduce_all_internal(const bool value) {
  cinn_block_reduce_internal_kernel(value, true, cinn_warp_shuffle_all_internal);
}
// block reduce any internal
__device__ inline bool cinn_block_reduce_any_internal(const bool value) {
  cinn_block_reduce_internal_kernel(value, false, cinn_warp_shuffle_any_internal);
}
...
#define block_shuffle_kernel(name, op, init_value)                               \
  __device__ inline decltype(init_value) block_shuffle_##name(const decltype(init_value) *buf, int line, int stride) { \
    auto val = init_value;                                                              \
...
block_shuffle_kernel(all, cinn_all, true);
block_shuffle_kernel(any, cinn_any, false);
