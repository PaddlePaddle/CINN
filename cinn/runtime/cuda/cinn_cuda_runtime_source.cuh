...
#define cinn_warp_shuffle_internal_kernel(TYPE, value, op)                                \
  ...
  if (threadIdx.x < lane) tmp_val = op(tmp_val, __shfl_down_sync(mask, tmp_val, 16, 32)); \
  if (threadIdx.x < lane) tmp_val = op(tmp_val, __shfl_down_sync(mask, tmp_val, 8, 32));  \
  if (threadIdx.x < lane) tmp_val = op(tmp_val, __shfl_down_sync(mask, tmp_val, 4, 32));  \
  if (threadIdx.x < lane) tmp_val = op(tmp_val, __shfl_down_sync(mask, tmp_val, 2, 32));  \
  if (threadIdx.x < lane) tmp_val = op(tmp_val, __shfl_down_sync(mask, tmp_val, 1, 32));  \
