...
void CodeGenCUDA_Dev::Visit(const ir::Call *op) {
  ...
  if (!op->read_args.empty()) {
    for (int i = 0; i < op->read_args.size() - 1; i++) {
      ...
      if (arg.as_var()) LOG(INFO) << "Args is var " << arg.as_var()->name;
      if (arg.as_buffer()) LOG(INFO) << "Args is buffer " << arg.as_buffer()->name;
      if (arg.as_tensor()) LOG(INFO) << "Args is tensor " << arg.as_tensor()->name;
      if (arg.as_tensor()) {
