...
TEST(Cuda, cuda_const_vectorize) {
  Expr N1(128);
  Expr N2(512);
  Expr N3(7);
  Expr N4(8);
  Placeholder<float> A("A", {N1, N2, N3, N4});
  Expr const_value(float(2.11));

  auto C = Compute(
      {N1, N2, N3, N4}, [&](Var i1, Var i2, Var i3, Var i4) { return const_value / A(i1, i2, i3, i4); }, "C");

  auto stages = CreateStages({C});
  // int prod_size = std::accumulate(Ashape().begin(), A.shape().end(), 1, std::multiplies<int>());
  stages[C]->Fuse(0, 1);
  stages[C]->Fuse(0, 1);
  stages[C]->Fuse(0, 1);
  stages[C]->Split(0, 4);
  stages[C]->Split(0, 1024);
  stages[C]->Vectorize(2, 4);
  stages[C]->Bind(0, "blockIdx.x");
  // stages[C]->Bind(1, "blockIdx.x");
  stages[C]->Bind(1, "threadIdx.x");

  std::string ctf = "blockIdx.x";
  ctf.back()      = 'x' + 2;
  LOG(INFO) << "Bind index:" << ctf;
  Target target = common::DefaultNVGPUTarget();
  // Target target = common::DefaultHostTarget();
  auto func = Lower("divide_vectorize", stages, {A, C}, {}, {}, nullptr, target);
  ...
  auto* A_host = common::BufferBuilder(Float(32), {N1.as_int32(), N2.as_int32(), N3.as_int32(), N4.as_int32()})
                     .set_random()
                     .Build();
  auto* B_host = common::BufferBuilder(Float(32), {N1.as_int32(), N2.as_int32(), N3.as_int32(), N4.as_int32()})
                     .set_random()
                     .Build();
  ...
  CUDA_CALL(cudaDeviceSynchronize());
  tester("divide_vectorize", args.data(), args.size());
  CUDA_CALL(cudaDeviceSynchronize());
}
