...
void Graph::SaveSourceCode(int group_idx, const std::string& code) {
  ...
  std::string code_path = viz_path_ + "/source_codes/";
  if (!MakeDirectory(code_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
    LOG_IF(WARNING, viz_count_ == 1) << "Failed to make directory: \"" << code_path
                                     << "\", the CINN subgraph's source code will not print.";
    code_path.clear();
    return;
  }

  WriteToFile(utils::StringFormat("%s/group_%d.cc", code_path.c_str(), group_idx), code);
}
...
void Graph::SavePTXCode(int group_idx, const std::string& ptx) {
  ...
  std::string ptx_path = viz_path_ + "/ptx_codes/";
  if (!MakeDirectory(ptx_path, S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH)) {
    LOG_IF(WARNING, viz_count_ == 1) << "Failed to make directory: \"" << ptx_path
                                     << "\", the CINN subgraph's ptx code will not print.";
    ptx_path.clear();
    return;
  }
  WriteToFile(utils::StringFormat("%s/group_%d.ptx", ptx_path.c_str(), group_idx), ptx);
}
