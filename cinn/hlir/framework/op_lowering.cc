...
std::vector<ir::LoweredFunc> OpLowerer::Lower(GroupPtr& group, bool apply_op_schedule, bool apply_group_schedule) {
  ...
  if (FLAGS_cinn_ir_schedule) {
    switch (group->op_pattern_kind) {
      ...
      case framework::kInjective:
        return LowerGroup(
            group, apply_op_schedule, apply_group_schedule, &OpLowerer::ElementwiseScheduleDetermineFunction);
      case framework::kReduction:
        return LowerGroup(group, apply_op_schedule, apply_group_schedule, &OpLowerer::ReduceScheduleDetermineFunction);
      case framework::kOutFusible:
        LOG(FATAL) << "Group Pattern Kind kOutFusible Is Not Implemented!";
      case framework::kNonFusible:
        return LowerGroup(
            group, apply_op_schedule, apply_group_schedule, &OpLowerer::NonFusibleScheduleDetermineFunction);
      default:
...
bool OpLowerer::ElementwiseScheduleDetermineFunction(Node* node) { return true; }
...
std::vector<ir::LoweredFunc> OpLowerer::LowerGroup(GroupPtr& group,
  ...
  std::vector<ir::Expr> func_bodies =
      LowerOps(nodes, &group_func_arg_tensors, &tensor_map, do_op_schedule, schedule_determine_func);
  ...
  return PostProcess(&ir_sch, group, tensor_map, &group_func_arg_tensors, do_op_schedule);
}
...
std::vector<ir::LoweredFunc> OpLowerer::LowerCustomCall(GroupPtr& group) {
  ...
  auto impl           = OpStrategy::SelectImpl(
      cinn_strategy[node->op()](node->attrs, op_func_arg_tensors, out_types, out_shapes, target_));
...
std::vector<ir::LoweredFunc> OpLowerer::PostProcess(ir::IRSchedule* ir_sch,
  ...
  if (!done_op_schedule) {
    ...
    for (auto& tensor_pair : tensor_map) {
      ...
      group_func_args.emplace_back(tensor_pair.second->buffer, ir::Argument::IO::kOutput);
    }
  }
...
std::vector<ir::Expr> OpLowerer::LowerOps(const std::vector<Node*>& nodes,
                                          std::vector<ir::Tensor>* group_func_arg_tensors,
                                          std::unordered_map<std::string, ir::Tensor>* tensor_map,
                                          bool apply_op_schedule,
                                          ScheduleDetermineFunction schedule_determine_func) {
  ...
  for (Node* node : nodes) {
    ...
    std::vector<ir::Tensor> op_func_arg_tensors =
        std::move(CollectInputTensor(node, group_func_arg_tensors, tensor_map, this->type_dict_, this->shape_dict_));
...
std::vector<ir::LoweredFunc> OpLowerer::DoOpLower(Node* node,
  ...
  // 1.Do compute
  common::CINNValuePack pack = op_impl->fcompute(common::CINNValuePack{cinn_inputs});
  ...
  for (int idx = 0; idx < pack.size() - 1; ++idx) {
    ...
    if (pack.size() - 1 > node_datas.size()) {
      // Some nodes may output multiple temp tensors in their Compute definition,
      // but only one output node_data in the graph, and we use id + "_0"/"_1" as key.
  ...
  std::vector<ir::LoweredFunc> funcs =
      lang::LowerVec("fn_" + node->id(), tmp_stages, *op_func_arg_tensors, {}, {}, nullptr, this->target_, true);
  VLOG(4) << "Lower op: " << node->op()->name << ", get " << funcs.size() << " LoweredFunc:\n";

  op_func_arg_tensors->clear();
  for (int idx = 0; idx < pack.size() - 1; ++idx) {
    CHECK(pack[idx].is_tensor());
    op_func_arg_tensors->push_back(pack[idx].operator ir::Expr().as_tensor_ref());
  }
...
// group schedule
ir::Expr OpLowerer::DoGroupSchedule(ir::IRSchedule& ir_sch,
                                    const GroupPtr& group,
                                    const std::unordered_map<std::string, ir::Tensor>& tensor_map) {
  // topological order.
