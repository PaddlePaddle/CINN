...
std::shared_ptr<OpStrategy> StrategyForScale(const framework::NodeAttr &attrs,
  ...
  framework::CINNCompute scale_compute([=](lang::Args args, lang::RetValue *ret) {
    ...
    Expr scale_e = common::make_const(A->type(), scale);
    Expr bias_e  = common::make_const(A->type(), bias);
    if (bias_after_scale) {
      out = Compute(
          A->shape, [=](const std::vector<Expr> &indice) { return scale_e * A(indice) + bias_e; }, tensor_name);
    } else {
      out = Compute(
          A->shape, [=](const std::vector<Expr> &indice) { return scale_e * (A(indice) + bias_e); }, tensor_name);
    }
