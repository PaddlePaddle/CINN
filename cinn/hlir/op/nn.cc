...
std::shared_ptr<OpStrategy> StrategyForReshape(const framework::NodeAttr &attrs,
  ...
  auto strategy = std::make_shared<framework::OpStrategy>();
  CHECK(out_type.size()) << "Out_type of reshape op is empty! Please check.";
  if (out_type[0] == Float(32)) {
    strategy->AddImpl(reshape_compute, reshape_schedule, "strategy.reshape.x86", 1);
  } else {
    LOG(FATAL) << "reshape op with dtype != float32 is not implemented yet!";
  }
  return strategy;
}
...
std::vector<framework::shape_t> InferShapeForReshape(const std::vector<framework::shape_t> &inputs_shape,
  ...
  if (attrs.attr_store.find("shape") != attrs.attr_store.end()) {
    ...
    LOG(FATAL) << "shape is not found!";
  }

  // accumulate the shape
  auto bp              = [](int x, int y) { return x * y; };
  auto acc_input_shape = std::accumulate(inputs_shape[0].begin(), inputs_shape[0].end(), 1, bp);
  // set shape exist -1
  int position = -1, acc_shape = 1;
  for (int idx = 0; idx < shape.size(); ++idx) {
    if (shape[idx] == -1) {
      // if position != -1
      if (position != -1) {
        LOG(FATAL) << "more than one dimension is set -1!";
      }
      position = idx;
      continue;
    }
  ...
  // if exist -1
  if (position >= 0) {
    shape[position] = acc_input_shape / acc_shape;
  }
...
CINN_REGISTER_HELPER(nn_ops) {
  ...
  CINN_REGISTER_OP(reshape)
      .describe("This operator implements the meta op Reshape.")
      .set_num_inputs(1)
      .set_num_outputs(1)
