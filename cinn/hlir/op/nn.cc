...
std::vector<shape_t> InferShapeForConv2d(const std::vector<shape_t> &inputs_shape, framework::AttrMapType &attrs) {
  ...
  if (attrs.find("padding") != attrs.end()) {
    padding = std::get<std::vector<int>>(attrs.at("padding"));
  }
  if (attrs.find("stride") != attrs.end()) {
    stride = std::get<std::vector<int>>(attrs.at("stride"));
  }
  if (attrs.find("dilation") != attrs.end()) {
    dilation = std::get<std::vector<int>>(attrs.at("dilation"));
  }
  if (attrs.find("data_format") != attrs.end()) {
    data_format = std::get<std::string>(attrs.at("data_format"));
  }
  CHECK_EQ(padding.size(), 2) << "The size of padding in conv2d op is not 2! Please check.";
  CHECK_EQ(stride.size(), 2) << "The size of stride in conv2d op is not 2! Please check.";
