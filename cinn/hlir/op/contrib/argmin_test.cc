...
TEST(GenerateCode_Cpu, Argmin_Keep) {
  ...
  lang::Placeholder<int> out("out", {n, out_c, h, w});
  poly::StageMap stages = poly::CreateStages({in});
  ir::Tensor res        = Argmin(in, axis, true, stages, "test_argmax_in");
...
TEST(GenerateCode_Cpu, Argmin) {
  ...
  poly::StageMap stages = poly::CreateStages({in});
  ir::Tensor res        = Argmin(in, axis, false, stages, "test_argmax_in");
  stages->InsertLazily(res);
