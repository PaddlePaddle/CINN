...
TEST(GenerateCode_Cpu, Argmax) {
  ...
  poly::StageMap stages = poly::CreateStages({in});
  ir::Tensor res        = Argmax(in, axis, false, stages, "test_argmax_in");
  stages->InsertLazily(res);
  std::vector<ir::LoweredFunc> funcs =
      lang::LowerVec("TestGenerateCodeCpu_Argmax", stages, {res}, {}, {}, nullptr, target, true);

  VLOG(6) << "Expr before CPU codegen:";
  VLOG(6) << funcs[0]->body;

  ir::Module::Builder builder("Argmax_Module", target);
  for (auto& f : funcs) {
    builder.AddFunction(f);
  }
