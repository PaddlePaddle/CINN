...
ir::Tensor Scatter(const ir::Tensor &A,
  ...
  auto res = Compute(
      ...
      [=](const std::vector<Expr> &indices) {
        ...
        auto idx = lang::CallExtern(extern_fun_name, {transpose_B, Expr(5), indices[pos_axis], Expr(0), Expr(1)});
  ...
  auto res = Compute(
      ...
      [=](const std::vector<Expr> &indices) {
        ...
        for (int i = 0; i < pos_axes.size(); ++i) {
          ...
          auto cur_idx = lang::CallExtern(extern_fun_name, {B, Expr(0), indices[0], Expr(i), Expr(1)});
          if (idx.empty()) {
...
std::shared_ptr<framework::OpStrategy> StrategyForScatterNd(const framework::NodeAttr &attrs,
                                                            const std::vector<ir::Tensor> &inputs,
                                                            const std::vector<Type> &out_type,
                                                            const std::vector<std::vector<int>> &output_shapes,
                                                            const Target &target) {
...
std::vector<std::vector<int>> InferShapeForScatter(const std::vector<std::vector<int>> &inputs_shape,
