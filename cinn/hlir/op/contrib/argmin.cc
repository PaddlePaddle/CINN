...
Tensor Argmin(const Tensor& in_tensor, const int& axis, const bool keep_dims, const std::string& output_name){
  ...
  CHECK_GT(real_axis, 0) << "Axis must be more than 0";
...
//    auto value = in_tensor(eval_indices);
//    auto update = ir::GT::Make(value, ir::Load::Make(ir::Tensor(max_value), {Expr(loop_var)}));
...
//    return ir::Load::Make(output, {shape[real_axis]-1});
  ...
  if (keep_dims) {
    ...
    CHECK_EQ(output_shape.size(), ndim);
  }else{
    output_shape.erase(output_shape.begin()+real_axis);
    CHECK_EQ(output_shape.size(), ndim-1);
  }
  if (output_shape.empty()) {
    output_shape.push_back(Expr(1));
    CHECK_EQ(output_shape.size(), 1);
  }
...
std::shared_ptr<framework::OpStrategy> StrategyForArgmin(const framework::NodeAttr &attrs,
  ...
  framework::CINNCompute argmin_compute([=](lang::Args args, lang::RetValue *ret) {
    ...
    std::string tensor_name = UniqName("T_Argmin_out");
    CHECK_EQ(arg_packs.size(), 1U) << "There should be 1 input args for argmin compute";
    Expr in_expr = arg_packs[0];
    CHECK(in_expr.as_tensor());
    Tensor in_tensor = in_expr.as_tensor_ref();
    auto out    = Argmin(in_tensor, axis, keep_dims, tensor_name);
