...
ir::Tensor Flip(const ir::Tensor &input, const std::vector<int> &axes, const std::string &name) {
  return cinn::hlir::pe::Reverse(input, axes, name);
}

std::shared_ptr<framework::OpStrategy> StrategyForFlip(const framework::NodeAttr &attrs,
