...
class GraphGroupInputFusePassCtx final : public InputFusePassCtx {
 public:
  ...
  const OpGroupList& PickConsumersWithSameInputs() const override { return groups_; }
...
bool WithoutLastDimInReduce(const api::Shape& inshape, const std::vector<int>& axes) {
...
static bool CanReduceFuseReduce(const OpGroupPtr& first, const OpGroupPtr& second) {
...
struct HorizontalFuseUtil {
  ...
  static bool ReduceFuseReduce(FusePassCtxT* ctx, const OpGroupPtr& src, const OpGroupPtr& dst) {
    return CanReduceFuseReduce(src, dst);
  }
};
...
class DefaultInputFusePass final : public InputFusePass {
 public:
  ...
  void operator()(InputFusePassCtx* ctx) const override {
    ...
    const auto& consumers = ctx->PickConsumersWithSameInputs();
    if (consumers.size() <= 1) {
      return;
    }
...
class GeneralFusionMergePassHelper : public FusionHelperBase {
 ...
 private:
  ...
  bool GeneralHorizontalFuse(const GroupPtr& producer) {
    ...
    using OpGroupSets                       = std::set<std::pair<OpGroupPtr, OpGroupPtr>>;
    ...
    const auto& GetFusableConsumerGroupList = [&]() -> GroupList {
      ...
      const auto& group_pair = *group_sets.begin();
      GroupList ret{group_pair.first.GetGroup(), group_pair.second.GetGroup()};
      return ret;
    };
    bool update = false;
    while (true) {
  ...
  bool IsDependency(const GroupPtr& producer_g,
    ...
    while (!candidates.empty()) {
      ...
      for (const auto& producer_and_list : candidate->producer_groups()) {
        ...
        const auto& producer = std::dynamic_pointer_cast<Graph::Group>(producer_and_list);
