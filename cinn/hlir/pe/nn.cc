...
std::vector<ir::Tensor> Conv2d_winograd_NCHW(const ir::Tensor &input,
  ...
  auto weights_dilation = Compute(new_weights_shape,
  ...
  if (pad_h == 0 && pad_w == 0) {
    ...
    input_pad = Compute(input_pad_shape,
                        [=](Expr nn, Expr cc, Expr yy, Expr xx) {
                          auto cond = lang::logic_and(
                              {yy >= pad_h, yy < input->shape[2] + pad_h, xx >= pad_w, xx < input->shape[3] + pad_w});
  ...
  auto input_tile                    = Compute(input_tile_shape,
  ...
  auto data_pack = Compute(data_pack_shape,
  ...
  auto bgemm = Compute(bgemm_shape,
  ...
  auto res = Compute(output_shape,
                     [=](Expr nn, Expr ff, Expr yy, Expr xx) {
                       return lang::ReduceSum(
                           input_pad(nn, rc, yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w) *
                               weights(ff, rc, ry, rx),
                           {rc, ry, rx});
                     },
                     output_name);
...
std::vector<ir::Tensor> Conv2d_NCHW_5D(const ir::Tensor &input,
  ...
  auto data  = Compute({batch, ic_chunk, h_in, w_in, ic_bn},
...
std::vector<ir::Tensor> Conv2d_NCHWc(const ir::Tensor &input,
  ...
  auto input_pad =
      Compute({batch, c_in_outer, h_in + 2 * pad_h, w_in + 2 * pad_w, c_in_inner},
      ...
      Compute(output_shape,
              [=](Expr n, Expr oc_chunk, Expr oh, Expr ow, Expr oc_block) {
                ...
                if (c_in == c_filter) {
                  ...
                  ic_outer = common::AutoSimplify(
                      ((oc_chunk * c_out_inner + oc_block) / c_out_per_group * c_filter + fc) / c_in_inner);
                  ic_inner = common::AutoSimplify(
                      ((oc_chunk * c_out_inner + oc_block) / c_out_per_group * c_filter + fc) % c_in_inner);
                }
                return lang::ReduceSum(
...
std::vector<ir::Tensor> Conv2d_NHWC(const ir::Tensor &input,
  ...
  auto weights_dilation =
      Compute(new_weights_shape,
              [=](Expr nn, Expr cc, Expr yy, Expr xx) {
                auto cond = lang::logic_and({(yy) % dilation_h == 0, xx % dilation_w == 0});
