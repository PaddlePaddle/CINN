...
std::vector<Expr> InferShapeLayoutTransform(const std::vector<Expr>& input_shapes,
                                            ...
                                            std::unordered_map<int, std::vector<int>>* split_index_map) {
  ir::Layout src_layout(old_layout);
  ir::Layout dst_layout(new_layout);
  int src_dim = src_layout.ndims();
  int dst_dim = dst_layout.ndims();
  std::vector<Expr> output_shape(dst_dim);
  // opt: check shape and layout size consistent,expand dim?
