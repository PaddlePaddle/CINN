...
class TestOp(unittest.TestCase):
    def test_op(self):
        a, b, c, c_target, *args = create_data()
        module = test_codegen()
        self.engine = cinn.ExecutionEngine()
        self.engine.link(module)
        matmul = self.engine.lookup("op_unittest")
        matmul(args)
        c_data = c.numpy()
...
def test_codegen():
    ...
    inputs = [a.to_tensor(), b.to_tensor()]
    ...
    res = strategy_map.apply_strategy("add", attrs, inputs, types, target)
