...
class TestTriangularSolveOpUnitDiagonal(TestTriangularSolveOp):
    def init_case(self):
        ...
        self.outputs = {
            "solution": np.array([[0.0], [-9.0], [5.0]]).astype(np.float32)
        }
        self.left_side = True
        self.upper = True
        self.transpose_a = False
...
class TestTriangularSolveOpDoubleFloat(TestTriangularSolveOp):
    def init_case(self):
        self.inputs = {
            ...
            "input2":
            np.array([[[0.79183686], [0.00260521],
                       [0.47066584]]]).astype(np.float64),
        }
        self.outputs = {
            "solution":
            np.array([[[7.19693364], [-13.30327987],
                       [-0.04562798]]]).astype(np.float64)
        }
        self.left_side = True
        self.upper = True
        self.transpose_a = True
