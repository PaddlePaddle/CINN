...
class TestLogicalRightShift(OpTest):
    ...
    def init_case(self):
        self.inputs = {
            ...
            "x": np.array([1690476611,   142184466, -1752569340,  1860589058, -1295695292,
        ...
        -1903975007, -1197706543,  1327016838,  -232019105]).astype(np.int32)
        }
        self.inputs = {
            # "y": self.random([1, 24], 'int32', low = 0, high=32)
            "y": np.array([20,  3, 12,  3,  0, 31,  0,  2,  6, 16,  1,  7,  6,  2, 19, 16,
         7, 17, 10, 15,  8,  9, 24,  4]).astype(np.int32)
        }
        self.outputs = {"out": np.array([[1612,    17773058,      620702,   232573632, -1295695292,
                  0, -1416770533,   952921202,     4441217,       33576,
         1735970258,    21804660,    58736042,   378758339,        5880,
              34885,     5506273,       14201,     3726311,       59195,
            9339813,     6049337,          79,   253934261]]).astype(np.int32)}
